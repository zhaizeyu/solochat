...
6. **安全聊天**  
   - Alice：安全面板 →「邀请开启」→ 登录密码 → 出现「取消邀请」  
   - Bob：刷新会话 →「同意邀请」→ 登录密码  
   - Alice：刷新会话 →「确认开启」→ 登录密码 → 出现「申请关闭」（确认后本端内存已解锁）  
   - Bob：刷新后若见「解锁会话」则解锁，否则已解锁；Alice 发密文，Bob 可见  
   - Alice「申请关闭」→ Bob「确认关闭」（勾选确认 checkbox）  
...
- `complete` 邀请后发起方内存已解锁；刷新页面后需再「解锁会话」
